...
| 院级审核 | `/achievement/collegeAudit` | 院级审核员审批：通过/驳回/退回修改 | 院级审核员 / 管理员 |
| 校级审核 | `/achievement/schoolAudit` | 校级审核员审批：通过/驳回/退回修改 | 校级审核员 / 管理员 |
...
|--------|---------|-----------|
| 0 | 草稿 | 教师编辑、提交 |
| 1 | 院审中 | 院级审核员：通过 / 驳回 / 退回修改 |
| 2 | 校审中 | 校级审核员：通过 / 驳回 / 退回修改 |
| 3 | 已通过 | 归档（终态，不可变更） |
| 4 | 已驳回 | 教师可重新提交 |
| 5 | 退回修改 | 教师修改后重新提交至院审 |
...
2. 📈 **审核状态统计** — 柱状图（草稿 / 院审中 / 校审中 / 已通过 / 已驳回）
...
| `sys_role` | system | 角色表（admin / teacher / CollegeAudit / SchoolAudit 等） |
...
> 其他角色账号（教师 / 院级审核员 / 校级审核员）需登录后通过「系统管理 → 用户管理」手动创建，并分配对应角色。
